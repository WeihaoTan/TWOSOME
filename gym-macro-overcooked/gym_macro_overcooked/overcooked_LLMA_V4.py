...
from .overcooked_V4 import Overcooked_V4
...
class Overcooked_LLMA_V4(Overcooked_V4):
    ...
    def __init__(self, grid_dim, task, rewardList, map_type = "A", n_agent = 2, obs_radius = 2, mode = "vector", debug = False):
        ...
        if map_type == "A":
            ...
            self.macroActionName = ["get tomato", "get plate 1", "go to knife 1", "deliver", "chop"]
        else:
            self.macroActionName = ["get tomato", "get plate 1", "go to knife 1", "go to knife 2", "deliver", "chop", "go to counter"]
            ...
            if not self.macroAgent[idx].cur_macro_action_done:
                ...
                if self.macroActionName[macro_action] in ["go to knife 1", "go to knife 2"] and not agent.holding:
                    ...
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                        self.macroAgent[idx].cur_macro_action_done = True
                elif self.macroActionName[macro_action] in ["get tomato"]:
                    target_x, target_y = self._findPOitem(agent, macro_action)

                    macroAction2ItemName = {"get tomato": "tomato"}
                    if self._calDistance(agent.x, agent.y, target_x, target_y) == 1:
                ...
                if self.macroActionName[macro_action] in ["get tomato"]\
                    or self.macroActionName[macro_action] in ["get plate 1"]:
                        target_x, target_y = self._findPOitem(agent, macro_action)
                        macroAction2Item = {"get tomato": self.tomato[0], "get plate 1": self.plate[0]}
